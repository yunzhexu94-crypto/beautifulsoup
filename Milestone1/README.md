...
As required by the project description, I attempted to process a gigabyte-scale XML file. The scripts failed to process the file, consuming all available system memory before crashing. This is the expected behavior for a DOM parser like BeautifulSoup, which loads the entire document tree into memory at once. 
